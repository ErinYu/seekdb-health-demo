...
def _build_conclusion(
    ...
    if sem_dist is not None:
        if sem_dist > 0.15:
            parts.append(
                f"🧠 **语义层面**：执行日与未执行日的日记描述差异明显"
                f"（向量距离 {sem_dist:.3f}），你的主观感受在两种状态下确实不同。"
            )
        elif sem_dist > 0.05:
            parts.append(
                f"🧠 **语义层面**：两种状态的日记描述有一定差异"
                f"（向量距离 {sem_dist:.3f}），主观感受有所区别。"
            )
        else:
            parts.append(
                f"🧠 **语义层面**：两种状态的日记描述在语义上非常接近"
                f"（向量距离 {sem_dist:.3f}），主观感受差异不明显。"
            )
